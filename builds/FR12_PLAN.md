...
_Slice 1 PUSHED + CHROME-VERIFIED 2026-07-19 (origin `34ea490`; see BACKLOG "FR-13a SLICE 1"): the reusable **source-constrained spell grant-child** (the source sibling of the tag-constrained slice-5 one) is built and harness-locked, and Scaletrix's 10 spells are fully named + sourced from Darryl's sheet (the ASK-DAMO item is resolved). Disintegrating Beam + Gravity Well now render as Arcane-source-filtered child pickers under Innate Power._

_Slice 2 PUSHED + CHROME-VERIFIED `1117fea` 2026-07-19 (baseline `34ea490`; builder-only; see BACKLOG "FR-13a SLICE 2"). **FR-13a is COMPLETE; Phase 1 done.** The three parts: (a) Command childed under the Fiendish Magic ancestry trait via a new `cgtrait:<i>` grant-child parent path; (b) the two Spellcaster-path spells (Dispel Magic, Telekinesis) source-filtered in place by a canonical `source: Arcane` on the flat entry (NOT childed under the path - path spells come from the caster-rank count, and a fake grant would double-count; in-place filtering delivers the same off-list-cleared payoff, a flagged design call); (c) the Sorcerous Origin sub-choice is now an explicit `source_choice` node that DRIVES the Intuitive-Magic children's source filter. Payoff delivered: all three flat Arcane picks (Command / Dispel Magic / Telekinesis) lose the "(current, off-list)" tag. catalog_verify 90/90, builder_verify PASS (7 new checks). With FR-13a done, Phase 1 (sourcing machinery) is complete; NEXT is Phase 2 (full spell/maneuver legality data) or Phase 3 (class coverage) per section 4._
...
**Phase 3. Class coverage (FR-12).** _Progress: 9 of 13. Sorcerer 2026-09-24 (base L1-L6); Wizard 2026-09-24 (base L1-L6 + 3 subclasses); Cleric 2026-09-25 (base L1-L6 + 3 subclasses); Bard 2026-09-26 (base L1-L6 + 3 subclasses). The data-only claim failed 8 to 10 ways each time, see the Sorcerer, Wizard, Cleric and Bard notes in `BACKLOG_DONE.md` Chores. The roster is now one edit (`class_spines.yaml`) plus a `catalog_build.CLASS_CONFIG` row._
