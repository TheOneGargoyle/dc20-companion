...
| FR-8 | Choice-bearing talents/subclasses need pickers/slots (epic) | feature | engine+catalog+builder | P1 | in progress (slice 1 done 2026-07-17) |
| FR-9 | Expand ancestry traits into slots (like maneuvers) | feature | builder | P2 | ready (Runt data resolved) |
| FR-10 | Echo next-level preview text into the level's section header | feature | builder | P3 | done (2026-07-17) |
...
| FR-16 | Show DR (PDR/MDR/EDR) on the Companion character page | feature | companion | P3 | Part A done (2026-07-16); Part B (toggle-aware) ready |
...
- **Part B (toggle-aware / buffed DR).** Make the DR line react to active toggles the way PD already does (`curPD()` / the "PD (buffed)" swap). Bonan's Rage toggle reads "half Phys/Elem dmg" = PDR Half + EDR Half while raging, but toggles currently only carry a `pd` delta. Give toggles an optional DR payload and add a `curDR()` that merges base gear DR with any active toggle DR - **"take the stronger", not additive** (DR does not stack: half + half is still half; a numeric vs "half" takes whichever reduces more). So Rage lights up "DR: PDR Half · EDR Half" while active and clears when it drops. Companion-only.
