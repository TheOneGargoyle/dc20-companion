...
|----|-------|------|------|-----|--------|
| FR-12 | Add full DC20 class + ancestry data coverage | feature | engine+catalog+builder | P2 | IN PROGRESS (Phase 3: 6 of 13 classes, Sorcerer 2026-09-24; `FR12_PLAN.md` Phase 3-4) |
...
**Pushed as `5a56242` on 2026-08-22 and verified against origin file by file; the deploy ran
(`Build: 2026-08-22 11:15 AEST · 5a56242`) and a live `rule` chip was clicked.**
...
**BUG-52. The Damage Calculator ignores Impact.** `general-rules.md:965`: *"Impact: The Weapon deals +1 damage on Heavy Hits."* The calculator already has the hit-grade buttons (Hit / Heavy / Brutal / Beyond), so this is a **grade-conditional +1 that is simply missing**, and it under-reports exactly like BUG-37 did. **It is not niche: 4 of the 6 PCs carry an Impact weapon** (Bonan's Baleful mace and glaive, Minimus's Longpole, Runt's Pact Staff, Xanwyn's short sword). **Checked before filing: there was no existing item for it.** Shape it like the rest of FR-25: a per-character flag in `damage_addons.yaml` beside `base`/`base_note` (the base weapon is already per-character there), applied in `dmgSync` when grade >= Heavy. Note Bonan carries two Impact weapons and Tan's Greatsword is not one, so a per-character flag matching the stated base weapon is the honest v1.

**FR-52. Three roll-section additions, all rules-cited rather than assumed.**

- **Versatile +2** (`general-rules.md:1002-04`): *"This Weapon can be wielded with 1 or 2 hands. When you wield the Weapon with 2 hands, you gain a +2 bonus to Hit using it."* So it is an attack-roll bonus, conditional on grip, and belongs as a toggle rather than baked into a character's attack number.
- **Flanking +2** (`general-rules.md:787-94`): *"You gain an additional +2 to your Melee Attack if the target is Flanked."* Melee only, and it is a table-state toggle, not a character property.
- **Help Die d6 and d4** (`combat.md:339-45`): the **Multiple Help Penalty** decays the die d8 > d6 > d4 on repeat Help Actions in a turn, and dice granted by other sources decay independently. The roll grid currently hard-codes one Help Die per character (Tan's is d8), so the decayed sizes cannot be rolled at all.

Design note: the first two are **modifiers applied to an existing roll**, not new roll buttons, so they want a small toggle row above the roll grid that adds to whatever is rolled next (and only to Melee Attacks for Flanking). Doing it as six more buttons would multiply the grid.

**BUG-52 and FR-52, both DONE 2026-08-15, one commit.** The filed shapes above are what shipped, with three deliberate departures.

**It is SIX of six, not four, and getting there took two passes.** The filed row named Bonan, Minimus, Runt and Xanwyn. Scaletrix's gear card also carries a **Mage Staff of Impact**, which the first pass caught. **Tanrielle it got wrong**, because the filed row said her Greatsword was not an Impact weapon and I encoded that as a harness assertion instead of checking. Darryl spotted the missing row on his own sheet within minutes of the handover. `general-rules.md`'s **Weapon Table** has **Greatsword: 2 S, Two-Handed, Heavy, Impact**, and her Longbow carries it too.

**The lesson, and it is the reusable half of this item: the gear cards are not the source of truth for stock weapon properties, the Weapon Table is.** The cards restate properties only when a magic item ADDS one (Bonan's Heavy Glaive, which the table lists WITHOUT Impact), so reading cards alone under-counts and reading the table alone over-counts. Both harnesses now read the table directly: `catalog_verify` asserts the Greatsword row still says Impact, and asserts the **Light Crossbow** row still does NOT, because Minimus's toggle ships OFF on the strength of that.

**Impact ships as a grade-gated toggle, not a silent flag, and that is the whole design.** Impact rides a **weapon**, but the calculator's Base Damage is per-character and editable, so a silent per-character +1 would over-report the moment anyone swung something else. Three of the six have a base that is **not** the Impact weapon (Minimus's base is the Arcane light crossbow and his Impact weapon is the Longpole; Runt's and Scaletrix's bases are spell bolts), so the toggle **defaults ON only where the Impact weapon IS the stated base attack**, which is Tanrielle, Bonan and Xanwyn. The other three ship OFF and are one tap. The def carries `when: heavy`, and `dmgSync` adds it only at grade >= Heavy; below that the button stays lit and reads **"+1 (needs Heavy)"** rather than quietly contributing nothing. **The gate is >= Heavy, not == Heavy**, on `core-rules.md:2323`: *"Brutal Hits are considered Heavy Hits for purposes of effects that trigger on a Heavy Hit."*

**The default is derived, not declared twice.** `default_on` sits in the catalog beside a new `weapon:` field, and both harnesses recompute it by matching the weapon's own name against that character's `base_note`. So it is not a check mirroring the literal it checks (trap 2, and the BUG-37 lesson): renaming a `base_note` out from under the pairing **fails the build**. Negative-tested by flipping Bonan's flag, which failed with the expected message.

**FR-52 added three controls, not six buttons.** Versatile and Flanking are `togglebtn`s above the grid that ride the **next Attack roll only**; `rollModTotal()` returns zero for anything that is not an Attack, so a Spell Check can never pick up a melee flanking bonus. The Help Die is now **live state**: the baked `'d8'` seeds `helpBase`, a chip steps it d8 › d6 › d4 › d8 for the Multiple Help Penalty, and the grid button relabels and rolls the current size. **The grid is still 9 buttons.**

**Verified in a browser, not by reading.** Bonan reads 2 at Hit, 4 at Heavy (`base 2 + Heavy +1 + Impact +1`), 5 at Brutal, and 3 at Heavy with Impact toggled off. Minimus reads 4 at Heavy with no Impact term. Tanrielle has no Impact row. Help stepped d8 › d6 › d4 › d8 and rolled the shown size. An Attack at +5 with both toggles read 19 where a Spell Check read 15. Zero console errors.

**FR-52 follow-up, 2026-08-15: the Help Die ladder now carries a d10.** Darryl spotted that the shipped `HELP_STEPS = ['d8','d6','d4']` would be wrong within a session or two. **Expert Commander at L5** (`classes.md:1207`): *"When you help a creature within your Commanding Aura, your Help Die starts at a d10."* That is the specific overruling the general d8 start, and the decay is then plain RAW off the ladder, so **no GM call is needed** (Darryl, 2026-08-15). The ladder is `['d10','d8','d6','d4']` and `helpBase` decides where a character enters it, so **the L5 flip is a one-word data change** in Minimus's baked roll entry, not a code change. At L4 all six enter at d8 and d10 is unreachable, which a new check asserts. **Honest limit: the d10 is conditional on being inside the Aura and the tool does not model that**; outside it his die is a d8, which is simply the next step down. Baseline moves **791 -> 792**.

**Minimus's Bolster advice was wrong, corrected 2026-08-15, and the correction is a BUG-37 in prose.** The card read *"Bolster before you spend. The Help Die refunds +2 SP, which pays for the Call. Fuel first."* Darryl challenged it. The Commander regen is *"Once per Round, regain up to half your max SP when you grant a creature a Help Die"* and is **capped at max SP**, so the gain is `min(ceil(max/2), max - current)`. At his max of 3 that is 2 from 0 or 1 SP, 1 from 2, and **nothing from 3**. Fuel-first is therefore right only when he is already low, and actively wasteful when he is not: at 3 SP, spending 2 before Bolstering turns a 3 SP pool into **5 SP of spend for the Round**.

**The threshold is DERIVED, not a number.** `threshold = max - ceil(max/2) = floor(max/2)`, so it is 1 at max SP 3 and becomes **2** the moment he reaches max SP 5. **Darryl caught the first draft of the fix hard-coding "1", which is `cap: 2` all over again on a surface where nothing asserts prose.** The card now states the derived rule with today's value in brackets, and **`builder_verify` (38) parses that bracketed number out of the artifact and compares it to `floor(baked max SP / 2)`**, so his next level-up fails the build instead of leaving Kristian reading a stale number at the table. Negative-tested by flipping the bracketed value. Baseline **792 -> 794**.

**The Reaction-Bolster combo is CONFIRMED RAW and is now the lead line on the card (Jesse, 2026-08-15):** *"Given the wording on reaction I'd say that's raw."* `combat.md` l.568-70 permits an own-turn Reaction *"in response to another creature's Reaction"*, Commander's Call has the ally attack **as a Reaction**, and Bolster fires *"whenever a valid target makes an Attack"*, so the carve-out is met. The canonical intended case is Parry against an Opportunity Attack, and this is the same shape. It is also the practical answer to the whole problem: **Call first, then Bolster as a Reaction to the Called attack**, so the Help Die lands on an attack already at ADV and the refuel arrives after the spending. Kristian had been playing it that way. It went to Jesse open, came back the same day, and went onto the card **flat with no provenance note**, the pattern the Titan Grip revert set hours earlier. **One thing stays off: the advice to hold a Bolster for an ally's turn is simply wrong**, since holding while full means he failed to spend on his own turn, which is the actual error.

**New harness section (38), and it asserts the ARTIFACT.** `builder_verify` now **builds the player Companion** and reads the baked `PARTY_DERIVED` back out of it, because a template reading a key the catalog never bakes passes every source-string check and still ships a broken calculator, which is the shape both BUG-52 and BUG-37 had (trap 3). 14 checks, about 2 seconds, so **the baseline moves 777 -> 791**. `catalog_verify` gained the rules grounding for all four properties plus the derivation check; its banner still says 90/90 because that number is hard-coded, which is now **CH-17**.

**CH-17. `catalog_verify`'s headline count is a string literal. DONE 2026-08-15.** Its PASS banner printed *"engine oracle holds (90/90 checks...)"* from a hard-coded `print()`. BUG-52 added 14 real assertions and the banner still read 90/90, so the number quoted as the regression baseline in `CLAUDE.md` and both starters could not move and could not fail.

**The fix turned up something better than the bug.** `git log -S` traces the literal to `f0f3857` (2026-07-12), where it was **copied out of section (1)'s own live output**. So **90/90 was never a wrong number, it was a wrong SCOPE**: it is the derived-stat oracle across the six ledgers, asserted at `catalog_verify.py:120`, and the banner presented it as a count of the whole harness. The real assertion count is **1725**.

**Both numbers now print, labelled.** `expect()` increments a counter; the banner reads `PASS - 1725 check(s), 0 failures. Engine oracle 90/90 derived-stat checks ...` and the FAIL path reads `FAIL - N of 1725 check(s) failed`. **The 1725 is DATA-DEPENDENT**, since adding a catalog row or a character moves it, so a change in it is a prompt to look rather than a regression on its own. **90/90 remains the fixed, asserted baseline** and is the one to quote. Negative-tested on both paths.

**The generalisable half:** a figure hand-copied out of another line's output stops tracking that line the moment it is pasted. The same shape as BUG-37's mirrored cap and as encoding a filed claim as an assertion. If a number is worth printing, compute it where you print it.

**FR-53 and FR-51, both DONE 2026-08-14, minutes after being filed.** Conditions and Roll are `<details id="secConds"/"secRoll" open>` now, the same pattern every other block uses, and **they default to OPEN so nothing changes for anyone who does not touch them**. **Their collapsed state persists** via `initSections()` and the existing `sget`/`sset`, because a section that re-opens on every load is not worth collapsing at a table. FR-51's chip now wears the active pill's colours with its left corners squared off, so the pair reads as one control instead of a pill followed by a stray dash.

**Verified in a browser:** both open by default, 30 pills and 9 roll buttons still render, collapsing Conditions survives a reload while Roll independently stays open, conditions still toggle correctly while the section is collapsed (the content stays in the DOM), and the chip computes to the red active background. Zero console errors.

**2026-09-23 Expertise batch: BUG-20 + FR-39 + BUG-49 (+ BUG-55 found on the way), one push.** Design A (picker variants) and one raise per trade, both Darryl's calls.
- **The choice is data on the trait entry**, `expertise: {kind: trades, target: Herbalism}`, copied from the variant the picker emitted. `build_engine.expertise_raises()` is the one reader (engine point/limit maths, allocator, plan-level caps). Free Level step = -1 point, cap +1; skills now get the discount too.
- **Engine problems:** a hand-written non-purchase `limit_raise`, Expertise plus a point purchase on one target, two Expertise raises on one target, an Expertise target with no mastery row. The limit check is now strict (+1 at most).
- **Catalog:** Human Skill Expertise and all four Trade Expertise rows modelled; Dwarf limited to Crafting or Services via new `trade_categories` in `skills_trades.yaml`, which catalog_verify (2e) asserts partitions `trades` and matches the `core-rules.md` headings. `coverage.py` EFFECT_KEYS swaps the never-used `limit_raise` for `expertise`. Burn-down 4 -> 2.
- **BUG-49:** `_TODO_CANON_OK` deleted; the todo set is keyed on (list, name). Mutation-checked: restoring the hand-written cap fails 2 checks, an Elf-sourced pick against an Elf todo fails.
- **BUG-55:** re-targeting `Attribute Increase (might)` to `(agility)` kept `attr_might`, because `base_name` strips the target. Targeted rows now compare the whole pick.
- **FR-39:** `rep.derived["points"]`, rendered as `#stpts` / `#lgpts`. New builder_verify (40), fleet asserts for every Expertise row, smoke (S7).
- **Verified:** all six ledgers levelled to L5 before and after: stats, problems, point prose, L5 caps and L5 plan options identical; only Herbalism's cap label changed. PARTY_DERIVED byte-identical. catalog_verify PASS 1899 (90/90), builder_verify PASS 872, smoke PASS.

**2026-09-23 L5 batch: CH-4 L5/L6 slice + FR-55, one push.** Party levels to 5, probably on Fri 2026-09-25.
- **L5 Expert features named for all five walked classes**, L6 marked `[]` (Talent + Path only). Two carry numbers: **Expert Spellblade `{disciplines: 1}`** (Xanwyn now gets a discipline child picker at L5; he had none) and **Expert Warlock `{hp: 2}` plus `per_held_pact_boon` riders** (Pact Weapon/Armor +1 maneuver each, Pact Spell +2 spells). Runt at L5: HP 20 (was 18), maneuver budget +2.
- The rider rule has ONE definition, `build_engine.class_feature_rider_grants`, used by the builder at level-up and by `catalog_verify` (2b). Weapon/Armor Property +1 and the typed-maneuver restriction are NOT modelled.
- **`tanrielle.yaml` L5 plan reshaped**: Spell Breaker moved from a separate discipline row into the Expert Spellblade row's `granted_disciplines`, so 2b can hold her to the catalog. Stats identical before and after.
- FR-46 gained `RT_FIXED_AT` (fixed features above L1, asserted against a grants-stripped catalog copy). New sections (39) and (39b); (5b) in catalog_verify cite-checks `rest_points.yaml`.
- Baselines: **catalog_verify PASS 1772, oracle 90/90; builder_verify --quiet PASS 828; builder_smoke --require PASS.** Mutations: dropping the rider call fails 39b, dropping the Long Rest HP keep fails 39.

**2026-09-23 follow-up (Darryl live-verify, same day):** Grit and Rest Points now share one row, the Spend control is a slim row ("Heal from Rest"), and the two hints are one line. Builder fixed rows render their note through `cleanNote`, which hides `Added in builder` (still in the ledger, `builder_api` reads it for removability) and the generated-row prefixes; class-feature notes lost their `classes.md l.N:` prefixes (now YAML comments). Asserted in (39)/(39b). builder_verify PASS 832.

**FR-55. Track Rest Points on the Companion.** Requested 2026-09-23 (relayed by Darryl). Nothing tracks them today: the Companion has HP, MP, SP, Grit and AP only, and its Long Rest button resets those. Rules (`general-rules.md` l.2083 to 2127): Rest Points equal HP max; spend 1 or more at the end of a Quick, Short or Long Rest to regain 1 HP each; all spent Rest Points come back at the Half Long Rest (first 4 hours). **SHIPPED shape (Darryl's calls 2026-09-23):** an RP row `x / HP max` beside Grit; a Spend control (Heal N, Heal all) usable any time, since Quick Rests are unlimited and Close Wounds / Recover spend outside rests; Long Rest split into **Half** (all RP back) and **Complete** (MP/SP/Grit/features; refills RP only if Half was skipped). **Complete Long Rest no longer restores HP**, which matches 0.10.5 (HP returns only through Rest Points); flagged to Darryl. No Short Rest counting (his call). Hooks come from `builds/catalog/rest_points.yaml` via `build_engine.rest_point_hooks`, matched on the ledger: Close Wounds (Tanrielle, target spends), Recover (Bonan), Flame rune Hearth +2 (Xanwyn), Font of Magic. Party hooks worth wiring: Xanwyn's Flame rune Hearth (+2 RP on a Short Rest, `classes.md` l.3091) and Bonan's Recover maneuver (`combat.md` l.1476, spend RP for HP in combat). Max must come from the engine's derived HP, so it moves at L5 with no hand edit. Open scope calls: split Long Rest into Half and Complete, and whether to count the 2 Short Rests per 24 hours.

**Re-asked 2026-09-23: persistence between sessions.** Still true as filed above, state persists per device and per browser. Fortnightly play is exactly the gap Safari's 7-day eviction bites, so if a player reports a wipe, ask which browser first.

**NOT A BUG: state already persists.** Darryl asked on 2026-08-14 whether per-character state (HP/MP/Grit/conditions) could persist between sessions. **It already does, and this was verified rather than assumed:** `sget`/`sset` (`template.html:418-19`) write to `localStorage` under a `dc20_` prefix, and a browser test set HP and MP, reloaded the page, and read them back unchanged (`dc20_curchar`, `dc20_cstate_tan`). So if a player is losing state, the cause is environmental, and the most likely one is **Safari on iOS/macOS evicting localStorage after about 7 days without a visit** (Intelligent Tracking Prevention), which would look exactly like "it forgets between sessions". Other candidates: private browsing, a different browser or device than last time, or clearing site data. **Worth asking WHICH player and WHICH browser before building anything.** If it does turn out to be Safari eviction, the fix is a different feature (an export/import or a shareable state string), not a storage change, since no web storage survives ITP eviction.

**FR-51. The stack "-" chip needs to read as a button.** Spotted by Darryl on the BUG-40 screenshot the night it shipped: the decrement chip sits between "Bleeding 3" and the next pill and looks like a separator, because it renders in the default pill style while its parent is highlighted. **Fix is one CSS line**: give `.pill.pilldec` the same `on` colouring as the active pill it belongs to, so the pair reads as one control. Not worth a deploy on its own; fold into the next Companion pass.

**CH-16. The builder's condition list is the duplicate BUG-40 left behind.** `builder_build.py` still carries the hand-written 12-name `CONDS` literal, used only by `_linkable` to decide whether a term gets a `rule` chip. The Companion's copy is now derived from `general-rules.md` (BUG-40), so the two disagree: **the builder thinks 12 conditions exist and the app knows about 28.** In practice that means a `rule` chip is silently missing for Doomed, Burning, Exhaustion and the rest wherever the builder renders one. **Do it the same way**, by deriving from the same source, ideally sharing `conditions_list()` rather than writing a second parser. **Not a five-minute job:** widening `CONDS` changes `_linkable` answers, so `builder_verify` (17)'s equivalence pass must be re-run and its `conds_lc` parse checked, and the baked `RULES_IDX` will change. That is exactly why it was left alone at the end of 2026-08-14 rather than tacked onto the Companion fix.
...
**FR-39. DONE 2026-09-23** (the Expertise batch): engine-structured `points`, rendered above the allocator and the language list.
...
3. **Bonan's Titan Grip double was genuinely open and is now CLOSED.** `09` carried "Titan Grip + Dual Wielding, two 2H weapons, one in each raging hand, 2nd attack MCP-free" as an open ruling, and the card was given a small-print caveat on 2026-08-14 rather than asserting it flatly. **Jesse confirmed it 2026-08-15 (via Darryl), 100 per cent**, so `09` line 40 is ticked and the card asserts the engine flatly again. **Darryl's call the same day: the card carries NO note either way.** A settled ruling is not news to the player whose sheet it is, and Ed does not need provenance in his own Optimization Workshop. **The rule this sets: a caveat earns its place on a player-facing card only while the question is OPEN. Once it settles, delete it, do not convert it to a confirmation.** The ruling record lives in `09` and `_SESSION_LOG.md`. His 10.5-EV boss turn stands as written in `12`.
...
6. **FR-49. DONE 2026-09-24** (see `BACKLOG_DONE.md` Features). Add `hp`/`mp`/`sp` to the engine's equipment-effect keys. Retired `DISPLAY_DELTAS` and closed
