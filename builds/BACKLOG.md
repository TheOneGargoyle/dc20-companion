...
**Two things deliberately kept OFF the card.** The Reaction-Bolster combo (Commander's Call has the ally attack *as a Reaction*, and `combat.md` l.568-70 permits an own-turn Reaction *"in response to another creature's Reaction"*) is **filed in `09` as an open question with Jesse**, because an open question does not belong on a cheat sheet. And the advice to hold a Bolster for an ally's turn is simply wrong: holding while full means he failed to spend on his own turn, which is the actual error.
