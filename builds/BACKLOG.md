...
| BUG-52 | Damage Calculator ignores the **Impact** weapon property (+1 damage on Heavy Hits) | bug | companion+catalog | P2 | DONE 2026-08-15, a grade-gated toggle on 5 of 6 (not 4); ON only where the base IS the Impact weapon |
...
**It is five of six, not four.** The filed row named Bonan, Minimus, Runt and Xanwyn. Scaletrix's gear card also carries a **Mage Staff of Impact**, so he got the add-on too. Read the gear cards before trusting a count, including one of ours.

**Impact ships as a grade-gated toggle, not a silent flag, and that is the whole design.** Impact rides a **weapon**, but the calculator's Base Damage is per-character and editable, so a silent per-character +1 would over-report the moment anyone swung something else. Three of the five have a base that is **not** the Impact weapon (Minimus's base is the Crossbow and his Impact weapon is the Longpole; Runt's and Scaletrix's bases are spell bolts), so the toggle **defaults ON only where the Impact weapon IS the stated base attack**, which is Bonan and Xanwyn. The other three ship OFF and are one tap. Tanrielle has no row at all, her Greatsword is not Impact. The def carries `when: heavy`, and `dmgSync` adds it only at grade >= Heavy; below that the button stays lit and reads **"+1 (needs Heavy)"** rather than quietly contributing nothing. **The gate is >= Heavy, not == Heavy**, on `core-rules.md:2323`: *"Brutal Hits are considered Heavy Hits for purposes of effects that trigger on a Heavy Hit."*
