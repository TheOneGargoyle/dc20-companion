...
| CH-17 | `catalog_verify`'s PASS banner hard-codes "90/90", so its headline count can never move | chore | tools | P3 | ready (found 2026-08-15; 14 new checks left the banner unchanged, see note) |
...
**CH-17. `catalog_verify`'s headline count is a string literal.** Its PASS banner prints *"engine oracle holds (90/90 checks...)"* from a hard-coded `print()`, not from the counter. BUG-52 added 14 real assertions on 2026-08-15 and the banner still read 90/90. So the number quoted as the regression baseline in `CLAUDE.md` and both starters **cannot move and cannot fail**, which is the same defect class as BUG-37's mirrored cap: a figure that looks like evidence and is not. Small fix, print the live pass count the way `builder_verify` does, then correct the baseline everywhere it is quoted. Do it before anyone reads 90/90 as proof of coverage again.
