...
2. **Bonan's card carries the caveat in the open.** His AoE-versus-focus rule is stated, with a small-print line saying the exact 2-target breakpoint is being re-checked and why. Better a player reads "this is under review" than silently plays a number we already suspect.
3. **Scaletrix's grater is presented as LIVE, not pending.** His doc still says it hinges on a Jesse ruling; that ruling landed on 2026-08-14 (Rolling Wild Fire does tick on forced movement, cited in `09`). Stale "pending" text in a player-facing app is worse than no text. **General lesson: these docs are from 2026-07-03/05, so check every "awaiting a ruling" line against the settled list before publishing it.**
