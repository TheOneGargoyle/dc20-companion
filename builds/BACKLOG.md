...
**FR-42 carry, added 2026-09-23 (Darryl):** when the sub-choice node exists, move Skill/Trade Expertise onto it. Today (BUG-20, design A) they are offered as per-target picker variants, about 28 per Trade Expertise list. Only the PICKER changes: the choice is already structured data (`expertise: {kind, target}` on the ledger entry), which the engine, allocator and harnesses read.
