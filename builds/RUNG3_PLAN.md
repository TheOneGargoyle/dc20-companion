...
5. *(5b)* **Party-stats wiring** (§10 build-item 3 — the "commit a ledger → the Companion updates" loop). **STATUS (2026-07-11): ✅ DONE / verified headless.** `companion-src/build.py` gained a party-derived stage: it imports the true `tools/build_engine.py`, replays all six `builds/<handle>.yaml` at `current_level`, and bakes the derived numbers (level, CM, attack/spell check, save DC, initiative, grit, HP/SP/MP, PD/AD) into the page as `PARTY_DERIVED`; `companion-src/template.html` merges them over `CHARS` at load — only engine-derived numbers are overwritten (incl. the L/CM subtitle, the Attack/Save DC/Initiative/MSL-SSL stat chips and the attack/spell/init roll buttons); all hand-authored prose, toggles, skills, saves and audit notes stay hand-maintained. Two documented Companion-display deltas ride on top (xan hp +2 Amulet of Health equipment overlay; runt pd +2 pending the CONFIRM-WITH-PHIL note in `runt.yaml`), so the baked page is value-identical to the previous hand numbers today and only moves when a ledger moves. Engine problems print as `ledger flag` warnings in the build log (the two known trade over-spends surface; the oracle stays `catalog_verify.py`). `deploy.yml` now installs pyyaml; the Action already watched `builds/**`, so a committed ledger genuinely redeploys the Companion with new numbers — verified by simulation (Tanrielle `current_level` 4→5 rebakes exactly the plan's L5 targets: CM 3, attack +7, save DC 17, HP 16, SP 4, MP 7, PD 19). The engine's `Report` gained a machine-readable `derived` dict (display-neutral; `builder.html` regenerated, harness + 66/66 PASS).
5. *(5c)* **Comment-preserving YAML export** (the step-5 thread's "(d)" v-next nicety). **STATUS (2026-07-11): ✅ DONE / harness-verified.** `export_yaml()` now re-anchors the source ledger's own YAML comments (header provenance block, EOL comments at their original column, comment blocks aligned under an EOL comment like tanrielle's pd note, section markers, blank-line spacing) onto the re-serialised file. PyYAML only - no new dependencies, so Pyodide and the harness run unchanged: the composer's line marks give format-neutral {line -> node path} maps for both the hand-written source and the fresh dump, and comments re-attach by path. The promote/undo rename `expected` <-> `expected_at_L<n>` is followed (the sheet-block comment rides the demotion); a comment whose anchor was edited away is collected under a clearly marked orphan section at the bottom rather than silently dropped; a merge failure can never block an export (plain-dump fallback with a console note). New-from-scratch exports get a generated 2-line provenance header. The localStorage WIP and self-serve text-reload paths keep the comments through resume. `tools/builder_verify.py` grew section (7), 27 checks: every comment line in each of the six ledgers survives an untouched export with NO orphan section and `safe_load(merged)` == ledger; edited-export and second-round-trip survival; the expected rename on promote; the orphan path; the scratch header - PASS, 66/66 oracle intact.
5. *(5d)* **Builder published at a static URL** (the infrastructure half of "hand the players the builder URL"). **STATUS (2026-07-11): ✅ DONE (pending the push).** `deploy.yml` now copies `builds/builder.html` verbatim into the Pages artifact, so the builder lives at `<site>/builder.html` next to the Companion - same bookmarkable-URL model, deep links (`?char=runt`, `?new=druid`) work as-is. No page changes needed: the builder is fully self-contained (fetch-first falls back to the bake on Pages), and the regenerate-on-change discipline plus the harness's bake-vs-source sha checks mean the published copy cannot silently drift. The guard step now also checks the builder page (present, ends in `</html>`, no GM strings). Remaining for (c): the one-page player how-to - see 5e.
5. *(5e)* **One-page player how-to** (the content half of "hand the players the builder URL" - closes §7's (c)). **STATUS (2026-07-11): ✅ DONE (pending the push).** NEW `builds/howto.html`: a static, self-contained one-pager in the builder's visual style covering the §5 self-serve workflow in four steps - open the builder URL (with the `?char=<handle>` / `?new=<class>` deep links and all six handles listed), make changes with the live green/red/amber check colours explained (plus the localStorage resume behaviour), export (respec vs CANON buttons, download or the copy box, send to Darryl via Discord), and "Darryl commits, the Companion updates" (re-validation before commit, the About-tab build stamp as the freshness check). Published by `deploy.yml` alongside the builder at `<site>/howto.html` (copied verbatim, same model as 5d); the guard step now checks it too (present, ends in `</html>`, no GM strings). No builder/engine/catalog changes - the page is pure prose + CSS, so there is nothing for the harness to cover; deploy simulation in the sandbox clone passed all guards. Players get ONE link that explains itself.
...
3. **`build.py` does not read the ledgers.** It bakes `rules_data` and `gm_data` from the rules markdown; the Companion's party data is maintained separately. For "commit a ledger, the Companion updates" to actually work, `build.py` needs a new step that runs the engine over the ledgers, emits the party stats, and bakes them in. This wiring closes the loop and is currently unbuilt. **Medium effort.** **STATUS (2026-07-11): ✅ DONE — see §7 step 5b.**
